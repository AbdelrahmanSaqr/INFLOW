...
const HomeHeader =()=> {
  ...
  // const btnClasses = `${classes.tasketestbtn} ${isScrolled ? classes.btnscrolled : ''}`;
  
  const [isHomePage, setIsHomePage] = useState(false);
  const [isArticles, setIsArticles] = useState(false);
  const [isFindCoach, setIsFindCoach] = useState(false);
  ...
  useEffect(() => {
    ...
    if( currentUrl.includes("http://localhost:3000/test-information") 
        || currentUrl.includes("http://localhost:3000/results") 
        || currentUrl.includes("http://localhost:3000/bookmeeting")
      ) 
    {
      setShowButton(false);
    }
  ...
  useEffect(() => {
    setIsArticles(window.location.pathname === '/articles' || window.location.pathname === '/articlepage');
    // setIsArticles(window.location.pathname === '/articlepage');
  }, []);
  useEffect(() => {
    setIsFindCoach(window.location.pathname === '/coach');
  }, []);
  useEffect(() => {
  ...
  const pages = [ 
    <span className={isHomePage ? classes.activeNavbarBtn : classes.navbarbtns} onClick={homeBtn}>
      <span>Home</span>
      {isHomePage && (
        <ProgressBar variant="custom" now={100} style={{ height: "4px", borderRadius: "20px" }} />
      )}
    </span>,
    <span className={isFindCoach ? classes.activeNavbarBtn : classes.navbarbtns} onClick={coachBtn}>
    ...
    <span>
      <span className={isArticles ? classes.activeNavbarBtn : classes.navbarbtns} onClick={articlesBtn} ><span>Our articles</span></span>
      {isArticles && (
        <ProgressBar variant="custom" now={100} style={{ height: "4px", borderRadius: "20px" }} />
      )}
    </span>,
      ...
      <AppBar position="static" style={{ background: 'rgba(255, 255, 255, 0) 100%' }}>
        <Container maxWidth="xl" >
          <Toolbar >
            ...
            <Button className={classes.navbarbtn} sx={{ flexGrow: 2, display: { xs: 'none', md: 'flex' } }}>
              {pages.map((page) => (
                <button
                  key={page}
